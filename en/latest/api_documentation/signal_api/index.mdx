---
title: "Signal API"
description: "Send post-call signals and custom conversion data to Invoca to attribute outcomes to the originating call and feed reporting, bidding, and audience use cases."
---
...
<Note>
If using **call_start_time** to match, instead of **transaction_id**, one or more of the following optional parameters is highly recommended. The optional parameters are ignored when using the **transaction_id**.
</Note>

**Optional** (when using `call_start_time`):
...
#### Example: Multiple signals using `transaction_id`
...
#### Example: Signals with custom data fields using `transaction_id`
...
#### Example: Only custom data fields using `transaction_id`
...
#### Example: Using `call_start_time` with all optional search fields
...
<Note>
The file upload user interface includes a timezone dropdown feature, which allows users to select which timezone this format is parsed in. Use the following to format Excel date cells: `yyyy/mm/dd hh:mm:ss.000 AM/PM`.
</Note>
...
- These fields can still be specified inline with each signal, but signals with different values for **partner_unique_id** will not be grouped together.
...
- Custom Data fields will not be displayed here, but will be applied to the Post Call Event transaction corresponding to the top-level **partner_unique_id**.
...
**Example: Creating two signals then updating one**

**First request** — creates both signals (valid due to unique `partner_unique_id`):
...
**Second request** — updates first signal:
...
<Note>
Even though this second request was an update to the first and will appear in reports as updating the first signal, a new signal transaction ID is returned. This is because a correction has been made to the first signal, and this new transaction ID is what will appear in webhooks and the Transactions API.
</Note>
...
<Note>
Any signals provided or associated previously with the call will also reflect these new Custom Data values.
</Note>
...
<Warning>
As signals are applied to the call, the response time of the API will increase with each signal added. In extreme cases, this can cause the request time to exceed the 120 seconds API timeout resulting in a 504 Gateway Timeout HTTP response.
</Warning>
