---
title: "Transactions API for advertiser users"
description: "Read call transaction data scoped to an advertiser (merchant) user in Invoca, with filtering, pagination, and export fields, using the Transactions REST API."
---
...
| `call_result_description_detail_managed_advertiser` | Call Result | Status of the transaction *(For the call for Signal Transactions Rollup)* |
...
| Field | Name in Reports | Description |
|-------|-----------------|-------------|
| `advertiser_call_fee_localized` | Fees | Advertiser Telecommunications fee associated with transaction *(Summed total amount for the call for Signal Transactions Rollup)* |
