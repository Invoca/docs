...
description: "Reference for the deprecated SolutionsJS (PNAPI) web integration snippet. New sites should use Invoca Tags, which internally rely on the InvocaJS library."
---
...
| numberToReplace | String or Hash | Default: `null` <br/><br/> Specify which number(s) to replace. Can accept one or many like so: `“888-999-1010, 777-888-9999”`. Optionally accepts a hash with the number as a key name, and campaignId as its value like so: <br/> `{ “888-999-1010”: ‘campaignIdHere’ }` |

| Optional parameters: |  |  |
| --- | --- | --- |
| autoRun | Bool | Default `true`<br/><br/>  determines whether Invoca’s web integration runs when the page loads.  If you have a single page application and would like to control when Invoca’s web integration runs, set autoRun to false and call Invoca.PNAPI.run() whenever you are ready.  Note: autoRun does not have to be false in order to call Invoca.PNAPI.run(). |
