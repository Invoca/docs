---
title: "InvocaJS Toolkit library reference"
description: "Reference for the InvocaJS Toolkit JavaScript helpers, including readInvocaData and setCookie, for advanced Invoca web integration and cookie management."
---
...
This function will search all query strings in the URL for a key name and return its value.
...
`inputFunction` must be a function, any value returned by this function will be considered true, so return null to try again.

`callbackFunction` must be a function, and is called with the found value or default value passed as an argument.

`defaultValue` is the value that will be passed into `callbackFunction` if `inputFunction` fails to return a value within the `maxWaitTime`.

`maxWaitTime` is the maximum amount of milliseconds to wait for `inputFunction` to return a value.