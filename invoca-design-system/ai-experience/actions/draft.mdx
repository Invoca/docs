...
- [Button](/invoca-design-system/components/actions/button) — Send/Apply as the single
  `contained` action per [Button's one-primary-per-region rule](/invoca-design-system/components/actions/button#constraints);
...
| **TITAN-DRAFT-02** | The send/apply control is the only `contained` Button in the draft's region; discard is `text` and comes first. | Matches [Button's one-primary-per-region rule](/invoca-design-system/components/actions/button#constraints) and the destructive-confirmation ordering — the safe path stays easiest to reach. |
...
| **TITAN-DRAFT-05** | Discarding a draft requires no confirmation dialog. | Nothing has been committed yet — matches [Destructive confirmation's own rule](/invoca-design-system/patterns/destructive-confirmation#constraints) that confirmation is for non-reversible actions only. |
