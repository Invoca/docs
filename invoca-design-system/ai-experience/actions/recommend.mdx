...
[Button's weight-follows-intent rule](/invoca-design-system/components/actions/button#constraints),
applied here at a different axis: weight should not overstate a proposal the system cannot
commit to on its own. It navigates to the real settings surface; it does not apply the change
itself.
...
| Uncertain | A recommendation built on a small sample or a short observation window is framed with weaker language ("early signal" rather than a flat claim) or suppressed below a stated evidence threshold. |
| Refused | A change was detected but no specific, actionable direction could be identified. The system says so rather than manufacturing a recommendation to fill the space. |
...
| Stale | The evidence window has closed or the business has already acted since the recommendation was generated. Every card carries a generated timestamp so the user can judge whether it's still current. |
...
3. **How sure is it, and does that change behavior?** Weak evidence changes the framing
   ...
   threshold. A confidence number with no behavior
...
| **TITAN-RECOMMEND-03** | The action button is never `contained`. | Matches [Button's weight-follows-intent rule](/invoca-design-system/components/actions/button#constraints): visual weight should not overstate a proposal the system cannot commit to. |
...
- What evidence-sufficiency threshold gates whether a
...
number alone. If that connection can't be made reliably, falling back to naming no action
rather than inventing one governs more of this pattern's real-world behavior than the happy
path does. Dated 2026-09-02;
