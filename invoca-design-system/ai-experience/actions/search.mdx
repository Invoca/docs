...
- If an interpretation chip carries any confidence-like signal, it's paired with text — never
  color alone, per [Color's own constraint](/invoca-design-system/foundations/color#constraints) on this.
...
| **TITAN-AISEARCH-06** | No confidence percentage is shown unless it would change what the user does next. | Matches [the overview's own constraint](/invoca-design-system/ai-experience/overview#constraints) on this — a number that doesn't change behavior is decoration. |
