...
- The AI-generated label pairs an icon or text with any visual treatment — never color alone,
  per [Color's own constraint](/invoca-design-system/foundations/color#constraints) on this.
...
|---|---|---|
| **TITAN-SUMMARIZE-01** | The summary is labeled as AI-generated at the point of display. | Matches [the overview's own disclosure-timing rule](/invoca-design-system/ai-experience/overview#constraints) — disclosure that requires the user to already suspect something isn't disclosure. |
...
| **TITAN-SUMMARIZE-03** | No confidence figure is displayed unless and until the API provides one. | Matches [the overview's own constraint](/invoca-design-system/ai-experience/overview#constraints) on this — inventing a number the source doesn't supply is worse than omitting it. |
...
| **TITAN-SUMMARIZE-05** | No model identifier, prompt text, or latency/cost figure is stated in product copy for this capability. | None is publicly documented; asserting one would be an invented fact, per [the overview's own rule against unsubstantiated claims](/invoca-design-system/ai-experience/overview#constraints). |
