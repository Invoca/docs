...
| 4 | Scope-setting line — what it can and can't do (welcome only) | Text content, no component | Yes — states both what the feature can and can't do |
...
| 7 | Next-step actions (no-results only) | [Button](/invoca-design-system/components/actions/button) | Recommended — per [EmptyState's own constraint](/invoca-design-system/components/data-display/empty-state#constraints) on resolving actions |
...
[Empty state copy](/invoca-design-system/content/empty-state-copy#constraints), the no-results
wording names the query; this page's own rule for the welcome wording similarly requires
naming what the feature can't do, not only what it can.
...
- The scope-setting line and the no-results heading are both real text, never baked into an
  illustration — matches [EmptyState's own rule](/invoca-design-system/components/data-display/empty-state#constraints) on this.
...
nothing" and "didn't finish" — that distinction is what this page's no-results-versus-cancelled
rule depends on, and it doesn't hold for every possible
