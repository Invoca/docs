...
- Whether steps come from the model's own internal reasoning trace or from a separately
  instrumented pipeline log is undecided — this determines whether this page's no-fabricated-steps
  rule is even enforceable, since a model that produces no inspectable intermediate state has no
  real steps to narrate.
...
this page's no-fabricated-steps rule is written to protect.
