...
| **TITAN-VERIFY-03** | Citation links are Link, never Button. | Navigation to a resolvable location follows [Button's own navigation rule](/invoca-design-system/components/actions/button#constraints). |
...
| **TITAN-VERIFY-06** | A citation is never styled or worded to imply verification has already happened ("confirmed," a checkmark icon). | That language claims a review step nothing performed. See [the overview's own rule against unsubstantiated claims](/invoca-design-system/ai-experience/overview#constraints). |
...
- Whether citation-to-passage mapping is produced by the model itself or a separate retrieval or
  alignment step is undecided — this affects how reliably this page's no-fabricated-citation
...
timestamp within it, which would make this page's passage-level citation requirement unbuildable
as written. It also depends on Invoca's data-retention rules for recordings and transcripts,
which determine how long a citation stays resolvable before this page's staleness-flagging rule
has to apply. Reverify when a model or pipeline
