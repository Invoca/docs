...
color never carries meaning alone, so any color treatment this page
...
- Color is never the only signal that content is AI-generated — this is
  [Foundations: Color](/invoca-design-system/foundations/color)'s own constraint applied
...
- If the treatment carries text (a tinted card with a summary inside it), that text meets
  WCAG 2.2 AA against the surface it sits on, exactly as
  [Color's own contrast requirement](/invoca-design-system/foundations/color#constraints)
  already requires for every other surface.
...
|---|---|---|
| **TITAN-AICOLOR-01** | An AI color treatment reuses an existing semantic or component token. It never introduces a new hue or literal for "AI." | Mirrors [Color's own no-local-override rule](/invoca-design-system/foundations/color#constraints) and its four-statuses rule — a new meaning met by a new hex is exactly what Titan's status model forbids, and an "AI purple" carries the same distinguishability problem at higher stakes. |
| **TITAN-AICOLOR-02** | Color is never the only signal that content is AI-generated. Pair with text or an icon. | Restates [Color's own constraint](/invoca-design-system/foundations/color#constraints) for this case: color vision deficiency and forced-colors contexts remove color as a channel entirely. |
...
own palette — is the same discipline
[Foundations' own no-local-override rule](/invoca-design-system/foundations/overview#constraints)
applies everywhere else: a new need is met by a proposal, never by a local override.
