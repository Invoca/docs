...
|---|---|---|
| Mark a surface as AI-made with a small, consistent icon | An icon from Titan's existing 154-icon set that fits, or a formal proposal to add one | An inline SVG or an ad hoc mark outside the closed set — [Iconography's closed-set rule](/invoca-design-system/foundations/iconography#constraints) already forbids this for every other icon in the product |
| Show the AI is actively generating | An outcome-state treatment on the relevant pattern page, per [Governors](/invoca-design-system/ai-experience/governors/stream-of-thought) | Reusing the identity glyph to also mean "in progress." One glyph now carries two meanings, which [Iconography's one-meaning-per-icon rule](/invoca-design-system/foundations/iconography#constraints) already prohibits for every other icon |
...
or from a formal proposal to extend it, exactly like any other icon in the
...
meaning it uses the `-alt` variant.
...
[Color's own constraint](/invoca-design-system/foundations/color#constraints) requires everywhere else — and with
...
- An icon-only identity mark has an accessible name stating what it is — "AI-generated," not
  a description of the glyph — the same requirement [Iconography's accessible-name rule](/invoca-design-system/foundations/iconography#constraints) places on every icon-only
  control.
- A decorative use of the glyph (appearing beside text that already discloses AI content) is
  hidden from assistive technology with `aria-hidden="true"`, per [Iconography's decorative-icon rule](/invoca-design-system/foundations/iconography#constraints).
- The glyph is never the sole carrier of the AI disclosure. Pair it with text, per
  [Iconography's icon-plus-text rule](/invoca-design-system/foundations/iconography#constraints) and the overview's disclosure requirement.

## Constraints

| ID | Constraint | Rationale |
|---|---|---|
| **TITAN-AIICO-01** | An identity glyph, if used, comes from Titan's existing 154-icon set or a formal proposal to it — never an inline SVG or an ad hoc mark. | Mirrors [Iconography's closed-set rule](/invoca-design-system/foundations/iconography#constraints). An icon outside the closed set has no size token, no color token, and no contrast guarantee. |
| **TITAN-AIICO-02** | An identity glyph carries exactly one meaning — "this is AI" — and is never reused to also mean "generating" or "uncertain." | Mirrors [Iconography's one-meaning-per-icon rule](/invoca-design-system/foundations/iconography#constraints). Those are separate glyphs owned by Governors and Trust builders respectively; reusing one glyph for two meanings makes both unreadable. |
| **TITAN-AIICO-03** | An identity glyph is never the sole signal that content is AI-generated. Pair with text — the feature's stated name. | Matches [Iconography's icon-plus-text rule](/invoca-design-system/foundations/iconography#constraints) and the overview's disclosure requirement — icon comprehension without a label is low, and worst for the users who need the disclosure most. |
| **TITAN-AIICO-04** | An icon-only identity mark carries an accessible name stating what it is, not what it looks like. | Mirrors [Iconography's accessible-name rule](/invoca-design-system/foundations/iconography#constraints) — a screen reader user needs the fact, not a shape description. |
...
Icons are learned, not read — [Iconography's one-meaning-per-icon rule](/invoca-design-system/foundations/iconography#constraints) exists because a
