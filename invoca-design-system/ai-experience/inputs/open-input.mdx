...
| Uncertain | The response states its own uncertainty in text (a caveat), and that caveat is what should change what the user does next — see [the overview's own constraint](/invoca-design-system/ai-experience/overview#constraints) on this |
...
3. **How sure is it, and does that change behavior?** Only a stated uncertainty caveat, not a
   number — see the Uncertain row above and [the overview's own constraint](/invoca-design-system/ai-experience/overview#constraints) on this.
...
- What counts as "grounded enough" to satisfy this page's grounding requirement — a single
...
the transcript, cite the record — well enough that this page's grounding requirement is
