...
- No control's effect is conveyed by color alone; a selected discrete control (e.g., "Full" in
  ...
  alone, per [Color's own constraint](/invoca-design-system/foundations/color#constraints) on this.
...
| **TITAN-TUNE-04** | An invalid or unsupported combination of control values is refused before running, naming the specific incompatible values. | "Invalid selection" tells the user something is wrong and nothing about what to fix — same reasoning as [Error messages' own actionable-resolution rule](/invoca-design-system/content/error-messages#constraints). |
