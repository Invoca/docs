...
<Warning>
  **Failure is the least-specified state.** Nothing in the component or the token layer
  expresses "this action failed", so every surface improvises. See
  [TITAN-GAP-22](/invoca-design-system/foundations/open-decisions#titan-gap-22).
</Warning>
...
<Warning>
  **Title case here means headline style, not "every word capitalized."** Lowercase short
  ...
  `textTransform: "capitalize"` doesn't know this exception and capitalizes every word
  unconditionally; for most short, verb-first labels the two happen to agree, but they
  diverge on any label containing a function word. See
...
<Warning>
  **Focus has no semantic token.** [TITAN-A11Y-04](/invoca-design-system/foundations/accessibility#constraints)
  requires a visible focus indicator, and the token layer supplies none for the general case —
  `border-focus` is specified and unimplemented. Button therefore inherits whatever the
  framework draws. Never remove it. See
...
- **The `large` size is reachable and undefined.** It is not themed, but nothing prevents
  passing it. [TITAN-BTN-10](#constraints) is a documentation rule with no enforcement.
