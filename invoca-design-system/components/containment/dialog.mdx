...
<Warning>
  **The package's `Dialog` export is a different, unstyled primitive, not this component.**
  ...
  documents. Import `Modal` for everything this page describes. See the
  [naming divergence](/invoca-design-system/foundations/divergences#titan-div-27).
</Warning>
...
| `spacing-4` | — | Title and footer padding, all sides — a different, unrelated value from the content area's own horizontal inset directly below |
...
- [Footers](/invoca-design-system/patterns/footers) — which surface owns a commit, and why a dialog's footer covers the dialog's fields and nothing outside it
