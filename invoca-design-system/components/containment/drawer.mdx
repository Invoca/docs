...
| `footerContent` | `ReactNode` | Optional. Pinned below the scrollable content, sticky to the drawer's bottom edge. |
...
- **`footerContent` is the drawer's own footer, and it commits the drawer's fields only** — not
  the view behind it, which keeps its own footer and is inert while the drawer is open. See
  [Footers](/invoca-design-system/patterns/footers) and
  [TITAN-FOOTER-02](/invoca-design-system/patterns/footers#constraints).
- **The footer is unpadded and unbordered, but not unstyled.** `footerContent` renders inside a
  wrapper that is sticky to the drawer's bottom edge, opaque (`background-default`), and stacked
  above the body — confirmed from source. It carries no padding and no border of its own, so
  spacing inside it and separation from the content above are the caller's responsibility.
...
| **Version** | 3.7.1 (hand-confirmed from `package.json` — check current for drift) |
...
- [Footers](/invoca-design-system/patterns/footers) — which surface owns a commit, and why a drawer's footer covers the drawer's fields rather than the view behind it
