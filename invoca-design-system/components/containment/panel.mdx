...
<Warning>
  **The package exports this as `Tile`, not `Panel`.** This page's name is the design
  ...
  and `TileTitle` — is the real import. See the
  [naming divergence](/invoca-design-system/foundations/divergences#titan-div-28).
</Warning>
...
**`TileFooter` holds a commit only when this container is the unit of commit.** On a view whose
whole purpose is configuration, the commit belongs to the view and this region holds a summary,
a timestamp, or nothing at all; when the container is the only configurable thing on a view
doing other work, the commit belongs here. See
...
- [Footers](/invoca-design-system/patterns/footers) — when `TileFooter` holds a commit, when it is only a summary region, and which surface owns the commit either way
