...
<Warning>
  **The default message breaks three of those rules.** With no props, the component renders
  **"No Records found!"** — title case, an exclamation mark, and the word *records*. Do not rely on
  the fallback; pass a title. Recorded as an issue against the component, not as guidance.
</Warning>
...
<Warning>
  **The supporting line renders as a heading, and it is not one.** `title` renders as an `h3` and
  `subTitle` renders as an `h6` — skipping two levels, and asserting that a sentence of supporting
  text is a document heading. This corrupts the heading outline of any page carrying an empty
  state. See [TITAN-DIV-22](/invoca-design-system/foundations/divergences#titan-div-22).
</Warning>
...
- **No rule for the action's prominence.** [TITAN-EMP-05](#constraints) requires one action and
