...
| 2 | Title | `title` | **Yes in practice** — omitting it leaves the component with no message, or the disallowed fallback copy |
...
<Warning>
  **There is no action slot, in code or in the design library.** Both define an empty state as
  ...
  This matters because a rule on [Table](/invoca-design-system/components/data-display/table#constraints)
  requires that an empty table offer the action that resolves it. That rule is satisfied by
  composition, not by this component.
</Warning>
...
| A very long `subTitle` | It wraps and pushes the region taller; there is no limit | Keep it to one sentence. |
...
- **The action, if there is one, is a link button — never a solid one.** Confirmed from
  source; see [Empty state copy](/invoca-design-system/content/empty-state-copy#constraints). Added this pass —
