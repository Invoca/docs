...
<Warning>
  **`disableBorder`, `disableHover`, and `bulleted` currently change nothing visually.** Each sets
  ...
  at all; it renders a plain native `<ul>`/`<li>` instead. See the
  [open divergence on List's unwired props](/invoca-design-system/foundations/divergences#titan-div-30).
</Warning>
...
records that as a [divergence](/invoca-design-system/foundations/divergences#titan-div-30) rather
than papering over it.

## Status

Package `@invoca/titan-core`, version **3.7.1**. Primary export: `List`. No deprecation marker or
