...
<Warning>
  **Do not copy this example's props.** Four of them do not exist in the shipped version:
  `pageSize`, `onPageSizeChange`, and `rowsPerPageOptions` were replaced by `paginationModel` and
  `pageSizeOptions`, and `unstable_cellSelection` is now `cellSelection`. The demo also declares
  `DataGridPro` as its subject and renders `DataGridPremium`.

  Verified against the installed package, not inferred. Until the story is corrected, treat the
  props table it publishes as unreliable and reach for the ones this page documents.
</Warning>
...
<Warning>
  ...
  *Table* is what people say in conversation, so it is the word this documentation leads with — and
  it is the one case where the reader-facing name comes from the team rather than from either
  artifact. Say **table** in a review and **grid** only when you mean the export. See
  [TITAN-DIV-10](/invoca-design-system/foundations/divergences#titan-div-10).
</Warning>
...
| Let someone edit many values freely, spreadsheet-style | Nothing — ask first | The system has no sanctioned editable-grid pattern. See [Gaps](#gaps-in-the-current-rules). |
...
<Warning>
  **A hovered row and a selected row are given the same fill.** While the pointer is over the
  table, "which rows did I select" is unanswerable — which matters most on exactly the pages that
  have bulk actions. Recorded as [TITAN-DIV-20](/invoca-design-system/foundations/divergences#titan-div-20).
</Warning>
...
| **TITAN-TBL-06** | Cells truncate; column headers do not. | A truncated header makes the whole column unreadable rather than one value. Shorten the header instead, and keep the full cell value reachable. |
...
**Also true and not yet recorded as either**: the published demo uses four props the shipped
version does not accept, and declares one export while rendering another (see
[Live example](#live-example)). No product adoption is recorded for the grid export in the
utilization data, so the adoption figures below cannot yet corroborate any usage guidance on this
page.
