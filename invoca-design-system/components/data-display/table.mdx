...
**The system says *table*. Code says `DataGridPremium`; the design library says *Data Grid*.**
...
**Use `DataGridPremium`, not `DataGridPro`.** Both ship in the package; `DataGridPremium` is
the one the system standardizes on and the one every list page should reach for.

| Term | Also called | The system uses | In code |
|---|---|---|---|
| **Table** | Data grid, grid, datagrid | **`table`** | `DataGridPremium` |
...
| **Selection bar** | Bulk bar, action bar, contextual header | **`selection bar`** | nothing — design intent specifies this; nothing in code ships it (see [Known issues](#known-issues)) |
...
| 6 | Pagination footer | the grid's own footer | No — where pagination lives is itself an open question (see [Known issues](#known-issues)) |
...
**Density** is out of scope for this documentation, so no compact row height is specified here
even though the grid accepts one.
...
| More columns than fit | The grid scrolls horizontally | Pin the identifying column so a reader scrolled right still knows which row they are on. |
| A cell's value is longer than its column | The text truncates | Truncate the cell, never the column header, and make the full value reachable. |
...
| Rows selected, then the filter changes | Selection persists for rows no longer visible | State the count, and say what it applies to before a bulk action runs. |
...
directly above the columns they narrow — which of the two actually owns them today is an open
question (see [Known issues](#known-issues)).
...
page from selecting a result set. **None of this exists in code** — it is the substance of
...
| ID | Was | Why retired |
|---|---|---|
| **TITAN-TBL-06** | Cells truncate; column headers do not. | This is `DataGridPremium`'s shipped themed default, not an independent per-table choice. Retired, not reversed. |
