...
<Note>
  **Exports, published variants, adoption, and status on this page are generated from Titan
  ...
  off. Contrast figures below are computed from the shipped token values. Disagreements between the
  design language and the shipped code are recorded in [Divergences](/invoca-design-system/foundations/divergences).
</Note>
...
| Announce something across the whole product | [Banner](/invoca-design-system/components/feedback/banner) | A page-level alert is about this page. Product-wide messages outlive it. |
...
**Also true and not yet recorded as either:** `severity` defaults to `success`, which no design
decision supports; every published example overrides the severity icon, so the default mapping is
demonstrated nowhere; and `AlertTitle` is exported with no treatment defined, so a titled alert is
buildable and unspecified.
