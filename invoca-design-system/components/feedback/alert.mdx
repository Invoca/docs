...
`severity` and `color` are separate props. The colour is how a reader identifies the severity
before reading, so the two must always agree — [TITAN-ALR-03](#constraints).
