...
|---|---|---|
| 1 | Box | Yes |
| 2 | Control label | No — an explicit accessible name is required when omitted |
...
|---|---|---|
| No visible label (e.g., a table row-selection checkbox) | The box alone | Must still carry an explicit accessible name |
| A select-all checkbox whose group is partially checked | **Undefined.** No mixed or indeterminate rendering exists in the design library. | See [Known issues](/invoca-design-system/components/forms/checkbox/open-issues#known-issues) |
...
option's label. A select-all checkbox sits at the head of the list it controls.

For a complete field rather than the bare control, compose through `FormInput` with
`type="checkbox"`, never by hand-assembling a raw Checkbox and FormControlLabel.
...
| **TITAN-CHK-04** | A disabled Checkbox is always accompanied by a reachable explanation of what would enable it. | The same reasoning a disabled Button already follows. |
...
[Known issues](#known-issues) for what that leaves unresolved).
