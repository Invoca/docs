...
type one. It removes the format ambiguity a typed date carries — the same reasoning behind
[Input's numeric-spinner constraint](/invoca-design-system/components/forms/input#constraints) applies here
...
|---|---|---|
| A day outside the selectable range (before a minimum or after a maximum date) | The `Disabled` day-cell state | Should be paired with a reachable reason it can't be chosen |
...
| Follow the system's date format everywhere this component renders one | Per [Dates and times](/invoca-design-system/content/dates-and-times#constraints) | A picker-specific format that disagrees with the rest of the product |
...
| **TITAN-DATEPICKER-02** | A range's start and end render in the same date format, per [Dates and times](/invoca-design-system/content/dates-and-times#constraints). | A range with two different formats on its two ends reads as two unrelated values, not one span. |
...
| **TITAN-DATEPICKER-04** | A disabled day cell is paired with a reachable reason it can't be chosen, somewhere in the composition. | Same reasoning as the disabled-state constraint on [Button](/invoca-design-system/components/actions/button#constraints) — a disabled cell alone announces nothing about why. |
