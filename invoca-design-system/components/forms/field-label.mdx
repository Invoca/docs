...
<Note>
  Story ID `verified` against the real Chromatic Storybook index — with a caveat.
  FieldLabel has no standalone story of its own (confirmed absent from the index, not
  just unchecked), which is consistent with this page's own claim that it is never used
  alone. This embeds the FormInput story that shows it composed with a field, which is
  the only form a live example of FieldLabel can take.
</Note>
...
| Both helper text and an error would apply at once | Undefined | Error should take the slot — not stated as a rule anywhere found. See [Gaps](/invoca-design-system/components/forms/field-label/open-issues#gaps-in-the-current-rules) |
