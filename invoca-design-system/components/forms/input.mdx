...
<Warning>
  **Placeholder is not a label.** It disappears the moment the field has a value or focus in
  some implementations, and is not reliably announced as a label by assistive technology. Every
  field needs a real, persistent label — FieldLabel's own page states this rule directly.
</Warning>
...
| # | Part | Required |
|---|---|---|
| 1 | Field label | Yes — a real, persistent label, per FieldLabel's own page |
| 2 | Left icon | No — confirmed in the design library as a named `Left Icon` variant, not just decoration |
| 3 | Trailing adornment (icon, clear action, unit) | No — at most one element in this slot |
| 4 | Placeholder | No |
...
| Disabled | Renders at the disabled tokens, not focusable | Pair with a reachable explanation, the same reasoning a disabled Button already follows |
| Read-only | **Undefined whether visually distinguished from disabled.** This page's own Constraints state what it should do; unconfirmed whether Titan does. | See [Gaps](/invoca-design-system/components/forms/input/open-issues#gaps-in-the-current-rules) |
...
`type="text"`, never by hand-assembling this component with FieldLabel.
...
| `aria-invalid="true"` and `aria-describedby` accompany an error, in addition to the visible colour and text change — never colour alone. |
| A read-only field stays focusable and keyboard-selectable; a disabled one does not. This is a platform default, not a Titan decision. |
...
| **TITAN-INPUT-03** | A disabled Input is always accompanied by a reachable explanation of what would enable it. | The same reasoning a disabled Button already follows — a disabled control announces nothing on its own. |
...
| **TITAN-INPUT-05** | An invalid Input signals it through the border and focus-ring colour **and** through error text — never colour alone. | Colour never carries meaning alone. |
