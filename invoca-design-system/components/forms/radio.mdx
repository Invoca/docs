...
<StorybookFrame story="components-radio--default" height={140} />
...
<Note>
  Anatomy diagram `pending` — `/images/components/radio-anatomy.png`. The images directory is
...
| A group with only one option | **Undefined.** A choice of one isn't a choice — see [TITAN-RADIO-05](#constraints). | See [Gaps](#gaps-in-the-current-rules) |
...
| A mandatory group with nothing selected on load | **Undefined** whether a default selection is required | See [TITAN-RADIO-06](#constraints) and [Gaps](#gaps-in-the-current-rules) |
...
- **No stated default-selection rule** for a mandatory Radio group — see
  [TITAN-RADIO-06](#constraints).
- **No rule for a group with only one option** beyond [TITAN-RADIO-05](#constraints) saying it
