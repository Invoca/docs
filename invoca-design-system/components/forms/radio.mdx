...
|---|---|---|
| No visible label | The circle alone | Must still carry an explicit accessible name |
| A group with only one option | **Undefined.** A choice of one isn't a choice. | See [Gaps](/invoca-design-system/components/forms/radio/open-issues#gaps-in-the-current-rules) |
...
| A mandatory group with nothing selected on load | **Undefined** whether a default selection is required | See [Gaps](/invoca-design-system/components/forms/radio/open-issues#gaps-in-the-current-rules) |
...
| **TITAN-RADIO-04** | A disabled Radio is always accompanied by a reachable explanation of what would enable it. | The same reasoning a disabled Button already follows. |
