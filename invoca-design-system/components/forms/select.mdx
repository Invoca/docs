...
| 2 | Field label | Yes — a real, persistent label, per FieldLabel's own page |
...
|---|---|---|
| No options available | **Undefined.** See [Gaps](/invoca-design-system/components/forms/select/open-issues#gaps-in-the-current-rules). | Should likely disable the trigger with a reachable explanation, matching the same reasoning a disabled Button already follows |
...
| A very long option list | **Undefined** cutover to Autocomplete or virtualization | Turn on Autocomplete past a certain length, per this page's own Constraints; see [Gaps](/invoca-design-system/components/forms/select/open-issues#gaps-in-the-current-rules) for where that length is |
...
`type="select"`, never by hand-assembling a raw, unwrapped Select.
...
| **TITAN-SELECT-06** | A disabled Select is always accompanied by a reachable explanation of what would enable it. | The same reasoning a disabled Button already follows. |
