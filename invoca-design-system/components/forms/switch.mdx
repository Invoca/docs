...
| 1 | Track | Yes |
| 2 | Thumb | Yes |
| 3 | Control label | No — an explicit accessible name is required when omitted |
...
| Disabled | One of several uncoordinated disabled-opacity values — see [Tokens](#tokens) | See the [open decision on disabled-opacity values](/invoca-design-system/foundations/open-decisions#titan-gap-32) |
| The setting change fails after being applied | **Undefined.** No component expresses a failed action — see the [open decision on failed actions](/invoca-design-system/foundations/open-decisions#titan-gap-22). | Surface the failure adjacent to the control and revert it, same reasoning as [Button](/invoca-design-system/components/actions/button#edge-and-failure-states) |
...
succeeded — see the [open decision on failed actions](/invoca-design-system/foundations/open-decisions#titan-gap-22).
...
| **TITAN-SWITCH-04** | A disabled Switch is always accompanied by a reachable explanation of what would enable it. | Same reasoning as the disabled-state constraint on [Button](/invoca-design-system/components/actions/button#constraints). |
