...
| Error | Invalid, warning, flagged | **`error`** | nothing — no error treatment ships for a tab today (see [Known issues](/invoca-design-system/components/navigation/tabs/open-issues#known-issues)) |
...
|---|---|---|---|
| 1 | The bar | `Tabs` | Yes |
| 2 | A tab's label | `Tab label` | Yes — 1–3 words, naming a section |
...
divider rather than drawing both.
...
is undecided (see [Known issues](/invoca-design-system/components/navigation/tabs/open-issues#known-issues)).
...
package, and the legacy stylesheets (see [Known issues](/invoca-design-system/components/navigation/tabs/open-issues#known-issues)).

## Edge and failure states

| Condition | What happens | What to do |
|---|---|---|
| Label longer than the bar allows | The label is truncated unless `wrapped` is set, which lets it run to a second line and grows the bar | Shorten the label to 1–3 words. A bar of unequal heights is the cost of a long label. |
...
| A section has an error inside it | Nothing. The tab looks the same as its siblings | No mechanism exists. Report the failure at page level instead. |
| A tab is disabled | Not focusable, announces nothing about why | Pair it with a reachable explanation. |
...
[page frame](/invoca-design-system/views/overview), including the fact that
`Header` cannot hold the bar today — it composes as a sibling instead.
...
See [Known issues](/invoca-design-system/components/navigation/tabs/open-issues#known-issues)
for the specifics of the pairing default and what the shipped panel does to block content.
Every application that needs a panel has written its own.
...
shipped panel wires it wrong (see [Known issues](/invoca-design-system/components/navigation/tabs/open-issues#known-issues)).
