...
<Warning>
  **This documentation previously stated the opposite rule for buttons.** An earlier pass of
  [Button](/invoca-design-system/components/actions/button) asserted sentence case as the rule and framed the
  theme's title-case rendering as a bug to work around
  ([TITAN-DIV-23](/invoca-design-system/foundations/divergences#titan-div-23), now corrected). zeroheight states
  plainly: **"Label text should be title case,"** with an explicit **"Label text should not
  be sentence case"** don't. That correction is reflected here and on the Button page.
</Warning>
...
<Warning>
  **This is not what the shipped theme currently does.** [Button's](/invoca-design-system/components/actions/button#content)
  theme applies `textTransform: "capitalize"`, which capitalizes *every* word with no
  exception for short function words. For a short, verb-first label ("Save changes," "Publish
  campaign") the two rarely disagree — most button labels don't contain an article or
  preposition long enough to notice. They would disagree on a label like "Save Changes to
  Draft," which the theme would render "Save Changes To Draft." This is a smaller, more
  precise version of what [TITAN-DIV-23](/invoca-design-system/foundations/divergences#titan-div-23) tracks now.
</Warning>
...
| **Field labels' casing is Proposed, not confirmed.** | If a real source states otherwise, [TITAN-CASE-05](#constraints) and [FieldLabel's](/invoca-design-system/components/forms/field-label#constraints) content rule both need to change together. | Open |
