...
| **TITAN-ERR-03** | Error message content states how to resolve the problem, not only that a problem exists. | "Invalid" tells the reader something is wrong and nothing about what to do next — the same reasoning as [FieldLabel's own constraint](/invoca-design-system/components/forms/field-label#constraints) on error text. |
