...
| A token exists but I cannot tell which to use | **A definition gap** | Still this page — the fix is a definition, not a new token. See [the open decision on undefined tokens](/invoca-design-system/foundations/open-decisions#titan-gap-02). |
...
| Surface | Why |
|---|---|
| **SCSS `$name`** | Frozen at 82 names by `legacy-surface-inventory.json` and asserted by test. New tokens never join it. See [the divergence this is recorded as](/invoca-design-system/foundations/divergences#titan-div-05). |
| **A dark or compact value** | Those axes are out of scope for the design language — see [the open decision on this](/invoca-design-system/foundations/open-decisions#titan-gap-01). The build accepts an extension; nothing in this documentation defines one. |
...
| **TITAN-TOK-03** | Every new token ships with a `$description` stating what it is for. | A token whose only guide is its name is a token two people will use differently. This is the rule that stops [the undefined-tokens gap](/invoca-design-system/foundations/open-decisions#titan-gap-02) growing. |
