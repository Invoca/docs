...
| Term | Means |
|---|---|
| `focus` | Appears in the specification as **both** a purpose and a state. `border-focus` is specified — and not implemented. |
...
| [2.4.7 Focus Visible](https://www.w3.org/WAI/WCAG22/Understanding/focus-visible.html) | AA | No semantic focus token exists. |
...
3. **White text on the success and warning fills fails badly.** A bold status fill needs dark
   text, and no token expresses that pairing.
...
| **TITAN-A11Y-06** | Every transition respects `prefers-reduced-motion: reduce`. | Vestibular disorders make large-area motion physically painful. Matches Motion's own equivalent rule. |
