...
<Warning>
  **Several standard pairings on this page fail WCAG AA** — measured, not estimated. See
  [Accessibility](/invoca-design-system/foundations/accessibility) for the full table and
  [TITAN-DIV-03](/invoca-design-system/foundations/divergences#titan-div-03) for what to do about it.
</Warning>
...
| A modal scrim | `overlay-default` | A black at some alpha — see the [known defect](#gaps-in-the-current-rules) |
...
<Warning>
  **Proposed — not yet signed off.** Titan ships four background tokens that function as
  surfaces, with no defined order and no rule pairing a border to the surface it sits on.
  The ordering below is a proposal. See
...
- **Surfaces have no defined layering order.** See
  [TITAN-DIV-07](/invoca-design-system/foundations/divergences#titan-div-07).
