...
| **prominence** | `primary` `secondary` `tertiary` `invert` | How much weight it carries. The design team's button vocabulary — no shipped token uses these words; code names the treatment (`contained`/`outlined`/`text`) instead. |
...
<Note>
  **Two terms overlap and the spec does not separate them.** `neutral` and `disabled` both
  ...
  value — one of several such collisions, see [Known issues](/invoca-design-system/foundations/color/open-issues#known-issues).
</Note>
...
disagree — every component token aliases a primitive directly rather than a semantic token
(see [Known issues](/invoca-design-system/foundations/color/open-issues#known-issues)).
...
<Note>
  ...
  alpha to a channel is never how a color variant should be derived; use the token for the
  intent instead.
</Note>
...
<Warning>
  ...
  *together*. Reaching for a plain status token where an `-alt` belongs is the most common
  source of contrast failures in Titan.
</Warning>
...
| A neutral, low-emphasis fill | `background-neutral` | `background-disabled`. They share a value and mean different things (see [Known issues](/invoca-design-system/foundations/color/open-issues#known-issues)). |
...
<Note>
  **`border-width-md` and `border-width-emphasis` are both 2px, but they are not the same
  ...
  Because the values are identical, the distinction is invisible on screen — another of
  several such collisions (see [Known issues](/invoca-design-system/foundations/color/open-issues#known-issues)).
</Note>
...
<Warning>
  **Color is never the sole carrier of meaning.** Every
...
- **There is no *semantic* focus token.** Three component-scoped rings ship —
  ...
  [Known issues](/invoca-design-system/foundations/color/open-issues#known-issues)).
