...
- **`background-neutral` and `background-disabled` are identical** (`#e7e9eb`). Two names, one
  value, different meanings — and one of seven such collisions, already recorded above.
- **`link-default` and `link-visited` are identical** (`#1c53e9`), so visited links cannot be
  identified — another of the same collisions, already recorded above.
- **`border-width-md` and `border-width-emphasis` are both 2px**, so their recorded distinction
  is invisible on screen — another of the same collisions, already recorded above.
...
- **The light-mode status contrast failures are untriaged.** Whether they are known and
  accepted or unnoticed is not recorded — already recorded above.
- **Surfaces have no defined layering order.** Titan ships four background tokens that
  ...
  it sits on. The ordering on the main page is a proposal, not yet signed off — already
  recorded above.
