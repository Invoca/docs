...
<Note>
  **This entry is a correction.** It previously described the focus token as undecided. The
  specification defines it, which changes the work from *make a decision* to *ship what was
  decided*.
</Note>
...
| | |
|---|---|
| **Code ships** | 34 variants defined in the type scale. 13 of them wired into the theme as base variants. 11 names exposed on `<Typography>`, which is the only way to reach them. |
| **Design intends** | The 11 `<Typography>` names are the design vocabulary. The other two sets are implementation surface. |
| **Follow today** | Use the 11 `<Typography>` variant names. Do not reach past the component into the underlying scale. |
...
<Note>
  **The mapping was the missing piece, not the grouping.** Concept-named pages without a
  recorded export mapping leave an engineer guessing what to import. `tools/component-concepts.json`
  records it, and the Exports table on each page is generated from it.
</Note>
...
<Note>
  **Corrected this pass.** An earlier version of this entry had the direction backwards —
  it asserted sentence case as the design intent and framed the theme's title-case rendering
  as the bug. zeroheight's Button page confirms title case is the actual rule: *"Label text
  should be title case"* / *"Label text should not be sentence case."* It also cited a
  specific usage count ("170 usages across 22 applications") that had no traceable source and
  has been removed. See [Capitalization and punctuation](/invoca-design-system/content/capitalization-and-punctuation)
...
<Note>
  **Found by cross-referencing two snapshot files, not source.** `density.json` records
  `form-field-focus-ring-error` from a light-mode probe; `colorSchemes.json` records both
  schemes' resolved tokens independently. Neither file states a relationship between the two
  numbers — the match was found by comparing them, not documented anywhere as intentional.
  This session had the resolved values but not the Titan checkout, so the coincidence is
  confirmed and its cause is not.
</Note>
