...
| **TITAN-ELV-05** | Never use shadow to indicate state — selection, error, or focus. | Selection and error have dedicated tokens (`background-selected`, `border-error`). Focus does not, but shadow is not the answer to that either. Shadow is spatial; overloading it makes both meanings unreadable. |
