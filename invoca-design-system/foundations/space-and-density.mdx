...
<Warning>
  **The specification stops at `spacing-9`; code ships `spacing-0` through `spacing-10`.**
  It also records no definition for any spacing step, so which gap each step is *for* is
  undecided — see [TITAN-GAP-02](/invoca-design-system/foundations/open-decisions#titan-gap-02) and
