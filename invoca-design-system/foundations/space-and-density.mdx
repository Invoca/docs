...
| ID | Was | Why retired |
|---|---|---|
| **TITAN-SPC-02** | Raw CSS uses `var(--titan-tokens-spacing-N)`, never `var(--titan-spacing-N)`. | Superseded by Foundations overview's own rule, which states the surface distinction without depending on density. |
| **TITAN-SPC-06** | Never scale `spacing-1` or `spacing-2` for compact. | Governs density behaviour, which is out of scope. Retired, not reversed. |
