...
Of the 13 theme-wired variants, 9 — every heading level above `h2`, both subtitle sizes,
`body2`, and `overline` — have no reachable `<Typography>` prop at all (see
[Known issues](/invoca-design-system/foundations/typography/open-issues#known-issues) for the
full accounting of how these three counts came apart).
...
<Warning>
  **Nine of the thirteen rows above have no reachable `<Typography>` prop**: `h3`, `h4`,
  ...
  `<Typography variant="h3">` from the underlying library renders correctly today, but design
  intent says not to — that is exactly the usage this page previously (incorrectly)
  recommended.
</Warning>
...
<Note>
  **Every row above that says "nothing reachable" is the same gap**, restated per use case —
  one open question: expose more of the scale, or accept eleven variants as final. Do not
  invent a local heading size to fill the gap; that is exactly the local-override problem
  overriding a variant's own properties exists to prevent.
</Note>
...
| **TITAN-TYP-05** | Never override `fontSize`, `fontWeight`, or `lineHeight` on a variant. | An overridden variant is a twelfth variant that no other consumer has and the audit cannot see. If none of the eleven fit, that is a token proposal — see the coverage gap in [Choosing a variant](#choosing-a-variant). |
| **TITAN-TYP-06** | Uppercase comes from the variant (`baseSmallAllCaps`, `baseSmallAllCapsBold`), never from `text-transform` or typed capitals. | Typed capitals are read letter-by-letter by some screen readers and cannot be un-cased for locales where the transform is wrong. Button's own uppercase transform is a separate, already-recorded case (see [Known issues](/invoca-design-system/foundations/typography/open-issues#known-issues)). |
...
- **The scale's `h6` step is 10px**, below the 12px floor most guidance recommends for
  ...
  directly off the scale, such as `EmptyState`'s `subTitle` (a component divergence recorded
  on that page) — treat it as a one- or two-word label only, never as anything a user must
  read carefully.
