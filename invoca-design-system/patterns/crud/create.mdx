...
<Steps>
  ...
  <Step title="Sequence — does order matter, and is there enough of it to show?">
    ...
    indicator — the design library has already drawn what that looks like, though nothing in
    code builds it yet. A single-section object never reaches this branch at all.
  </Step>
</Steps>
...
<Note>
  **"Modal" is this page's word for what Titan's design language calls Dialog.** Two of the
  ...
  themed surface — always imported from the package as `Modal`, never as `Dialog`. The
  ...
  the design term. Every link on this page to "Dialog" goes to the design-language page, and
...
|---|---|---|
| 1 | The trigger — [Button](/invoca-design-system/components/actions/button) `outlined`, or `text` if it's a secondary "add another" | Opens the chosen surface. Never `contained` unless creation is the page's single primary action. |
...
| 4 | [Button](/invoca-design-system/components/actions/button) `contained` — commit, `text` — cancel | Cancel is lighter weight and comes first, matching the trailing-primary order every other button pair in this system follows. |
...
| Modal's "More options" pressed | The surface transitions to the full page, carrying every value already entered. Nothing already typed is lost or re-asked. |
...
| A Wizard's step is left incomplete and the user navigates forward | Blocked with an inline message on the incomplete field. The step indicator this depends on is itself an undecided component — see [Related](#related). |
...
| **TITAN-CREATE-03** | A Wizard is used only when sections genuinely depend on each other in order. A form with several independent sections is one page with a Table of Contents, not a Wizard. | A Wizard forces linear traversal. Forcing it on independent sections adds clicks with no corresponding benefit — and the step indicator this depends on either way is itself an undecided component, not a reason to reach for it more often. |
| **TITAN-CREATE-04** | A Modal's escape hatch to a fuller surface carries forward every value already entered. It never restarts the form. | Discarding entered data to offer more room is a worse experience than not offering the room — the same reasoning CRUD's own choose-a-surface rules apply generally. |
| **TITAN-CREATE-05** | The commit button's label states what's being created, not a generic verb. "Create Campaign," not "Submit" or "Create." | At the moment of commitment, the button should say what it does without making the reader re-read the form — the same reasoning a destructive action's confirm button restates the action rather than saying "Yes" or "OK." |
| **TITAN-CREATE-06** | Closing or navigating away from a non-empty, unsubmitted create surface warns before discarding it, per [Destructive confirmation](/invoca-design-system/patterns/destructive-confirmation)'s existing rule for unsaved edits. An empty, untouched surface closes with no warning. | Warning on an untouched form trains the same dismiss-without-reading reflex that over-confirming anything else does — the warning has to be reserved for when it's actually protecting something. |
...
| ID | Kind | What |
|---|---|---|
| [TITAN-DIV-27](/invoca-design-system/foundations/divergences#titan-div-27) | Divergence | The package's `Dialog` export is not the design vocabulary's Dialog — the real component is `Modal`. |
| [TITAN-GAP-26](/invoca-design-system/foundations/open-decisions#titan-gap-26) | Open decision | No step indicator exists for multi-step flows. The Wizard shape this page recommends for sequential creation is a proposal against that undecided component. |
