...
| The reader is about to change the object's data, not just look at it | [Update](/invoca-design-system/patterns/crud/update) | A reading surface never writes — it opens Update's own surface instead. |
...
it has outgrown the row it's attached to, and belongs in a Drawer, where it has room and its
own scroll.
...
has for prefill, dirty state, and failure handling.
