...
[CRUD overview](/invoca-design-system/patterns/crud/overview#constraints) already states the
default: **the same object type uses the same surface for Create and Update, unless a stated
reason differs.** [Create](/invoca-design-system/patterns/crud/create#variations) already names
the one legitimate case where it does — this page resolves it rather than inventing a second
answer.
...
| A single field, no side effects on other data, no cross-field validation | [Inline editing](/invoca-design-system/patterns/inline-editing) | That page's own decision tree already answers this. Reopening a full edit surface for one field is the failure it exists to prevent. |
...
| 4 | [Button](/invoca-design-system/components/actions/button) `contained` — commit, `text` — cancel | Cancel is lighter weight and comes first, the same trailing-primary order every button pair in this system follows. Not present for a single-field edit — [Inline editing](/invoca-design-system/patterns/inline-editing#structure) has its own icon-only commit and cancel instead. |
...
| Saved filter graduates to a shared asset | The object's permanence changes after creation | Its edit surface moves from the lightweight one to the fuller one at that point, not before. |
| Quick edit reached from a Read surface | The reader is previewing the object in a Drawer or Detail view and wants to change one thing | Opens Update's own surface — Inline editing for a single field, the fuller surface otherwise. The Read surface itself never edits. |

## Anti-patterns

**Editing a Tag on a full page because "editing deserves more room than creating."** The Tag's
permanence hasn't changed just because it now exists — it never earned a fuller surface, so
its edit stays on the same one it was created on.
...
changed.** The reader loses their edit with no explanation — the same failure mode as a dialog
that closes on failure instead of showing what went wrong.
