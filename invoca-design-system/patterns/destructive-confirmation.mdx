...
down the attention the next, real one needs. That's why [TITAN-DESTROY-01](#constraints)
reserves confirmation for what undo cannot cover, rather than defaulting to a dialog whenever an
action is labeled "delete" — a product that confirms everything has, in practice, stopped
confirming anything.
