...
|---|---|---|
| Zero state | First-run, blank slate | **Zero state** — nothing has been created yet; reserved for the "hasn't been created" case specifically |
| No-results state | Empty search, no matches | **No-results state** — data exists; a filter, search, or query excluded all of it |
| EmptyState | *(the component both states below are built from)* | **EmptyState** — the illustration, title, and supporting-line component composed for either state |
...
| The region is still loading | [Loading & skeletons](/invoca-design-system/patterns/loading-and-skeletons) | Announcing "nothing here" and then filling the region contradicts itself. |
...
│                                               │
│              [ Create Campaign ]              │  ← link-styled, per Content
│                                               │
└─────────────────────────────────────────────┘
```
...
| Load still in progress | Never rendered by this pattern — see [Loading & skeletons](/invoca-design-system/patterns/loading-and-skeletons). |
...
|---|---|---|
| **TITAN-ZERO-01** | A region with nothing to show states which of the three causes applies. Never one generic message for all of them. | Follows the same rule EmptyState and its copy guidance already state, rather than restating it. |
| **TITAN-ZERO-02** | A failed load never renders through this pattern. Route it to [Error handling](/invoca-design-system/patterns/error-handling). | An empty table and a failed one look identical without a stated cause. |
| **TITAN-ZERO-03** | This pattern never replaces a region that is still loading. | Announcing "nothing here" and then filling the region a moment later contradicts itself. |
| **TITAN-ZERO-04** | The resolving action, when one exists, is exactly one action, styled as a link button, placed after the component. | Follows the same rule EmptyState and its copy guidance already state. |
| **TITAN-ZERO-05** | A table or list's empty state replaces the rows, not the container. Column headers and filter controls stay. | The reader whose filter matched nothing still needs the filter, in order to change it. |
...
| **TITAN-ZERO-07** | The illustration matches the case: a zero-state illustration is never reused for no-results, and neither is reused for a failure. | The illustration is read before the text, and one that contradicts the message costs the reader the time it takes to notice. |
| **TITAN-ZERO-08** | Copy names the real thing missing — campaigns, calls, contacts — never a generic word like "records" or "data." | Matches the same specificity rule Empty state copy already proposes. |
...
| Zero state with no permission to act | The reader could not create one even if they tried | Omit the action; state the reason instead — the same reasoning a disabled control follows when it needs a reachable explanation |
...
| ID | Kind | What |
|---|---|---|
| [TITAN-DIV-22](/invoca-design-system/foundations/divergences#titan-div-22) | Divergence | `EmptyState` renders its supporting line as a heading element, checked against source — a reader relying on the document outline encounters a second heading where design intent expects body text. |

## Gaps in the current rules

- Whether a zero state's resolving action should differ when the reader lacks permission to
  create the first record, beyond simply omitting the button — [Variations](#variations) names
  the omission but not what replaces it.
- What a no-results state should say when the search or filter is itself malformed (an invalid
  date range, a query with no valid tokens), as opposed to well-formed but unmatched.
- Whether a table's sortable, filterable controls should stay interactive during a zero state,
  or disable until content exists.

## Why it works this way

**Naming which of the three cases applies is the whole job, because the cost of getting it
wrong is asymmetric and silent.** A reader told "no results" when they've created nothing yet,
or told "get started" when their search just missed, doesn't see an error — they see an
interface that's confidently wrong, and correcting that requires them to already suspect it,
which most readers won't.

**The failed-load case is routed elsewhere rather than folded in as a fourth option, because
"empty" and "broken" are not degrees of the same thing.** This pattern never has to guess
whether an empty region is empty on purpose — that distinction belongs to
[Error handling](/invoca-design-system/patterns/error-handling), which
knows whether the load actually succeeded.

## Related

[EmptyState](/invoca-design-system/components/data-display/empty-state), the component this
pattern composes for both cases. [Empty state copy](/invoca-design-system/content/empty-state-copy),
the copy rules [Content](#content) follows rather than restates.
[Error handling](/invoca-design-system/patterns/error-handling), for the failed-load case this
pattern explicitly excludes. [Welcome / empty state](/invoca-design-system/ai-experience/actions/welcome-empty-state),
the AI-driven counterpart for an agentic panel's first run or empty result.
