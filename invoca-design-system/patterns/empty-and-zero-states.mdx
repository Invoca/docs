...
"empty" and "broken" are not degrees of the same thing.** [TITAN-ZERO-02](#constraints) exists
so this pattern never has to guess whether an empty region is empty on purpose — that
distinction belongs to [Error handling](/invoca-design-system/patterns/error-handling), which
