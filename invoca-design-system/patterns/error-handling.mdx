...
- What the exact threshold is before a "recurring failure" message ([TITAN-ERRHANDLE-09](#constraints))
  replaces a plain retry message — this page names the behavior without naming the count.
- Whether a background, silent retry ([TITAN-ERRHANDLE-06](#constraints)'s single automatic
  attempt) should ever be configurable per action, or is always exactly one.
...
it's made anyway because the alternative destroys real information.**
[TITAN-ERRHANDLE-03](#constraints) could be satisfied more simply by clearing the region and
showing only the error — the constraint exists because what the reader already had and what
just failed are both worth keeping visible at once.
