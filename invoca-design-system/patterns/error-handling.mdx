...
|---|---|---|
| Failure | Error, exception | **Failure** — a deterministic, exception-based non-success: a rejected request, a timeout, a failed load |
| Retry | Try again, reattempt | **Retry** — re-attempting the same failed action, either once silently or repeatedly at the reader's request |
| Partial failure | Mixed result | **Partial** — a load or batch action where some, but not all, of the attempt succeeded |
...
| Failure occurs inside a Dialog | The dialog **stays open**; an Alert appears inside it. Never close a dialog on failure — the same rule a destructive confirmation follows for its own. |
...
| N consecutive retries fail | The message states that the failure has recurred, not just that it failed again. |

## Constraints

| ID | Constraint | Rationale |
|---|---|---|
| **TITAN-ERRHANDLE-01** | A failure renders as close to the control that caused it as the composition allows. | Follows the same placement rule error copy generally follows, rather than restating it. |
| **TITAN-ERRHANDLE-02** | A failure inside a Dialog never closes the dialog. The Alert renders inside it. | Closing on failure discards the reader's context and they cannot tell whether it worked. |
| **TITAN-ERRHANDLE-03** | A partial load failure keeps whatever content already loaded. The failure is an Alert placed above it, never a replacement. | Replacing the content throws away what the reader had. |
| **TITAN-ERRHANDLE-04** | A failure with nothing to preserve — the first load failed entirely — uses EmptyState's `error` illustration with a Retry action. | The illustration must match the meaning; `error` is the one that says "this failed," not "there's nothing here." |
| **TITAN-ERRHANDLE-05** | The message states what happened and how to resolve it, never only that something went wrong. | Follows the same rule error copy generally follows, rather than restating it. |
...
- An Alert that appears after page load carries a live-region role so a screen reader user
  hears it.
- Severity is carried by the icon and the words, not only by color.
...
survived, and has to redo work that may already have gone through.
...
| ID | Kind | What |
|---|---|---|
| [TITAN-GAP-22](/invoca-design-system/foundations/open-decisions#titan-gap-22) | Open decision | No component in Titan expresses a failed action today, checked directly against source. There's a `loading` state and a `disabled` state, and nothing between them. Everything this page proposes is a way to improvise that gap consistently, not a description of a shipped primitive. |

## Gaps in the current rules

- What the exact threshold is before a "recurring failure" message replaces a plain retry
  message — this page names the behavior without naming the count.
- Whether the single automatic silent retry should ever be configurable per action, or is
  always exactly one.
- What a reader sees for a failure that recurs mid-retry, after they've already pressed Retry
  once by hand — whether the control re-enters `loading` identically or signals that this is a
  second attempt.

## Why it works this way

**Where a failure renders is decided by the same test at every level of the composition — is
there already a place, right next to what failed, to say so?** A control's own space, a Dialog,
a region above stale content — each is the closest available surface, chosen in that order,
rather than a single default like Toast that would be right for none of them.

**Keeping stale content on screen during a partial failure is the harder-to-build choice, and
it's made anyway because the alternative destroys real information.** The rule could be
satisfied more simply by clearing the region and showing only the error — it exists because
what the reader already had and what just failed are both worth keeping visible at once.

## Related

[Alert](/invoca-design-system/components/feedback/alert) and
[Toast](/invoca-design-system/components/feedback/toast), the two surfaces a failure renders
through. [EmptyState](/invoca-design-system/components/data-display/empty-state), for a full
load failure with nothing to preserve. [Empty & zero states](/invoca-design-system/patterns/empty-and-zero-states),
the sibling pattern for the non-failure case an empty region can also mean.
[Destructive confirmation](/invoca-design-system/patterns/destructive-confirmation), for a
dialog's own failure behavior this page follows rather than restates.
