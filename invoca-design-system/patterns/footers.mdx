...
| **TITAN-FOOTER-05** | A view-level footer is fixed to the viewport. A container-level footer is static within its container. | A container footer is already adjacent to the fields it commits. A view footer is a scroll away from wherever the reader is editing, and that distance is what makes a per-section button look like the fix. |
...
| | |
|---|---|
| **Constraints** | Proposed. `TITAN-FOOTER-01` through `-11`. |
| **Decided** | The configuration-view case and the container-scoped case are design decisions. |
