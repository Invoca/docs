...
| ID | Kind | What |
|---|---|---|
| [TITAN-GAP-30](/invoca-design-system/foundations/open-decisions#titan-gap-30) | Open decision | The design library specifies an `Error` display state for a tab; the shipped tab component has no equivalent, so an invalid field on an unopened tab has no per-tab signal — [TITAN-FORMVAL-05](#constraints)'s page-level summary exists specifically to work around this. |
...
**The page-level summary is a concession to a gap, not a preferred pattern.**
[TITAN-FORMVAL-04](#constraints) restricts it to long or tabbed forms specifically because a
summary above a form the reader can already see whole just repeats what the field-level message
already says. It earns its place only where TITAN-GAP-30 leaves no other way to point at an
invalid field the reader isn't looking at.
