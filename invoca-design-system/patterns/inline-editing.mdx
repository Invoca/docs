...
moment the field becomes editable — [TITAN-INLINE-03](#constraints) is deliberately unoriginal.

**The failed-save behavior in [TITAN-INLINE-05](#constraints) is argued from the same principle
as a failed destructive action, not derived independently.** Reverting silently on failure and
closing a dialog on failure destroy the same thing — the reader's belief that their input is
still there. Once one of those cases is settled, the other isn't a new decision; it's the same
decision applied to a different surface.
