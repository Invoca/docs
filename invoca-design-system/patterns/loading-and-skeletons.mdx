...
|---|---|---|
| Skeleton | Placeholder, shimmer, wireframe loader | **Skeleton** — a placeholder matching the known shape of incoming content |
| Spinner | Loader, throbber | **Spinner** — activity with no known shape and no measurable progress |
| Determinate | Measured, quantified | **Determinate** — a wait with a real, reportable amount of progress, shown by ProgressBar |
...
| Content already on screen is being refreshed | The data component's own loading affordance; keep the existing content visible | Replacing readable content with grey shapes takes away what the reader was looking at. |
...
| State | Behavior |
|---|---|
| Wait begins | If expected duration is well under a second, show nothing at all |
...
|---|---|---|
| **TITAN-LOADING-01** | Match the indicator to what's known: a real value → ProgressBar; a known content shape → Skeleton; neither → Spinner. | Each primitive communicates a different amount of information. Using the wrong one either overstates what's known or wastes information that was available. |
...
| **TITAN-LOADING-03** | A skeleton screen matches the real layout — same line count, widths, and spacing as the content it replaces. | Its only value is preparing the reader for a shape. A mismatch makes the page jump on load, which undoes the reason to use one at all. |
| **TITAN-LOADING-04** | One animation, one loading primitive, per region. Do not mix Skeleton's `pulse` with `wave`, or a skeleton with a spinner, in the same view. | Two loading rhythms in one region read as two different systems, not one thing loading. |
| **TITAN-LOADING-05** | A loading state always resolves — to content, an [EmptyState](/invoca-design-system/components/data-display/empty-state), or a handoff to [Error handling](/invoca-design-system/patterns/error-handling). It never simply stops appearing. | An indefinite loading state is a hang the interface is disguising as progress. |
| **TITAN-LOADING-06** | A skeleton screen used on more than one page is built once and shared, not re-implemented per application. | Three independent copies of the same `TabLoadingSkeleton`, 21 instances each, are the measured cost of not doing this — see [Known issues](#known-issues). |
| **TITAN-LOADING-07** | Under `prefers-reduced-motion: reduce`, pass `animation={false}` to every Skeleton on the screen. | The component does not honor the preference itself (see [Known issues](#known-issues)). This is a call-site workaround, not a fix. |
| **TITAN-LOADING-08** | The loading region carries `aria-busy` and announces completion once; the placeholder itself is hidden from assistive technology. | Grey shapes carry no information to a screen reader. The announcement belongs to the region, and only on completion. |
...
(see [Known issues](#known-issues)). Until the component handles this itself, pass
...
already happened three times over — see [Known issues](#known-issues). A composed skeleton
...
| ID | Kind | What |
|---|---|---|
| [TITAN-GAP-31](/invoca-design-system/foundations/open-decisions#titan-gap-31) | Open decision | Skeleton — the component this page leans on most — carries no theme overrides, no tokens, and no representation in the design library at all, despite being the most-used component in this area (132 usages across 13 applications). Three applications have independently built the same `TabLoadingSkeleton`. |
| [TITAN-GAP-08](/invoca-design-system/foundations/open-decisions#titan-gap-08) | Open decision | Reduced motion is unimplemented at the token layer — seven durations exist and none has a reduced variant, which is why [TITAN-LOADING-07](#constraints) is a call-site workaround rather than a fix. |
...
[ProgressBar](/invoca-design-system/components/feedback/progress-bar), the three primitives this
pattern chooses between. [Error handling](/invoca-design-system/patterns/error-handling), the
handoff once a wait exceeds its timeout. [Empty & zero states](/invoca-design-system/patterns/empty-and-zero-states),
