...
|---|---|---|
| Disclosure | Show more, expand/collapse | **Disclosure** — deferring genuinely secondary content behind an action, scoped to how much of the page it affects |
| Trigger | Toggle, expander | **Trigger** — the control that opens or closes the disclosed region; always labeled with what it reveals |
| Secondary | Advanced, optional | **Secondary** — used by a minority of users in a minority of sessions, per the heuristic above, not "feels advanced" |
...
| **TITAN-DISCLOSE-05** | Each Accordion trigger carries its own `id` and `aria-controls` pointing at its content, wired by hand. | Accordion does not pair these automatically — the only published example does it manually. Omitting it ships a disclosure control with no programmatic link to what it discloses. |
...
| ID | Kind | What |
|---|---|---|
| [TITAN-GAP-31](/invoca-design-system/foundations/open-decisions#titan-gap-31) | Open decision | Accordion carrying no group state of its own is a smaller version of the same per-application-rebuilding cost this gap already measures for skeleton screens — see [Anti-patterns](#anti-patterns). |

## Gaps in the current rules

- Whether disclosure state should persist across a page reload or a later session —
  [Behavior](#behavior) already flags this as undecided rather than guessing.
...
[Accordion](/invoca-design-system/components/containment/accordion), the component this
pattern's collapsible-section variant composes. [Tabs](/invoca-design-system/components/navigation/tabs),
for the settings-tab variant when the secondary area is large enough to be its own screen.
[Form validation](/invoca-design-system/patterns/form-validation), for the validation-error
