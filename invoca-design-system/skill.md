...
Titan documentation has four deterministic tiers, plus a fifth for anything AI-driven. Work
down the four in order. The page-level decision constrains everything below it, so starting at
the component is starting in the middle.
...
**A fifth area, `/invoca-design-system/ai-experience/*`, sits outside this ladder.** It covers
anything where the output is generated by a model rather than rendered from state, or where a
feature acts on the user's behalf. Do not resolve an AI feature through Views/Patterns/Components/
Foundations — start at [AI Experience overview](/invoca-design-system/ai-experience/overview)
instead. Its pages carry the same constraint-ID and register conventions as everything else,
plus four sections nothing else in this documentation has: **agency tier**, **outcome states**,
**disclosure & recourse**, and **evaluation** — see the overview for why. As of this writing
nothing in that section is built; treat every page there as proposal, not policy, except the one
line noted on [Actions: Summarize](/invoca-design-system/ai-experience/actions/summarize) about
Signal AI's shipped transcript summary.
...
| AI-driven features — agency, outcome states, disclosure, evaluation | `/invoca-design-system/ai-experience/*` |
