...
recent activity has stopped being a list and become a dashboard with a table on it. That is the
test this archetype exists on the far side of — a Dashboard is what that accumulation was
...
| Slot | Use | Constraint |
|---|---|---|
| Time-range or scope selector | `Controls` | Changes what the widgets show. Never in `Right`, which is for page-level actions, not view changes. |
| Export, customize, or another page-level action | `Actions` in `Right` | At most one primary. Many dashboards legitimately have none. |
...
| **TITAN-DASHVIEW-05** | A control that changes what the widgets show — a time range, a scope selector — goes in `Controls`, never `Right`. | It changes the view, not the data. |
| **TITAN-DASHVIEW-06** | At most one primary action in `Right`, and a dashboard may legitimately have none. | A page whose job is reading, not acting, often has nothing that rises to a page-level action. |
