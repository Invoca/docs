...
**Page-level actions on this specific record go here** — `Left` says what the record is, `Right`
says what can be done to it. This is the surface [CRUD:
...
At most one primary action. Collapse anything past three actions into a
[Menu](/invoca-design-system/components/actions/menu), per
[Views overview](/invoca-design-system/views/overview#header-right).
...
never an action or a filter. Selecting a tab changes the URL, so a specific section of a
specific record is linkable and survives a reload.

### Contents — attribute groups and related data

**The whole record is one `Contents` region.** When the page is tabbed, each tab's content is a
`TabPanel` inside that same `Contents` — the tabs partition one region, they do not create
additional ones.
...
A **page-level** footer applies only when the whole page is inline-editable as a unit — every
field open for editing at once, with one Save and one Cancel for the entire record. This is
rare.

**When one container on the page is configurable and the rest is read-only, the footer belongs
to that container, not to the page** — a static footer at the foot of the panel that holds those
fields, committing exactly them. The page carries none. That is the same rule as the page-level
case reaching a different answer, because the unit of commit sits inside the container rather
than spanning the view — see [Footers](/invoca-design-system/patterns/footers).

The far more common case, a single field the reader wants to change without leaving the page, is
[Inline editing](/invoca-design-system/patterns/inline-editing)'s territory: it commits
per-field, with no footer at all.
...
| **TITAN-DTLVIEW-02** | Page-level actions on the record — edit, delete — render in `Header` → `Right`, never beside `Title`. | `Left` says what the record is, `Right` says what can be done to it. |
| **TITAN-DTLVIEW-03** | The edit action opens whatever surface [CRUD: Update](/invoca-design-system/patterns/crud/update#choosing-a-surface-revisited) selects for this object type. This archetype does not choose the surface itself. | Matches List view's own relationship to Create — the surface decision belongs to the CRUD pattern, not the view that hosts the trigger. |
...
| **TITAN-DTLVIEW-05** | The record's attribute groups and related data share one `Contents` region. A tabbed body wraps each section in `TabPanel` inside that same region. | Tabs partition one region, they do not create a second. |
...
| **TITAN-DTLVIEW-07** | A page-level footer appears only when the entire page commits as one unit. When a single container is configurable, the footer belongs to that container; a single field's edit uses [Inline editing](/invoca-design-system/patterns/inline-editing) and no footer. | The footer goes on the surface whose boundary matches what it saves, and most attribute viewing has no whole-page commit to hold one at all — see [Footer](#footer). |
