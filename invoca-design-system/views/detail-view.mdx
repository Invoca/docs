...
than spanning the view — see [Footers](/invoca-design-system/patterns/footers) and
[TITAN-FOOTER-04](/invoca-design-system/patterns/footers#constraints).
...
| **TITAN-DTLVIEW-07** | A page-level footer appears only when the entire page commits as one unit. When a single container is configurable, the footer belongs to that container; a single field's edit uses [Inline editing](/invoca-design-system/patterns/inline-editing) and no footer. | Per [Footer](#footer) and [TITAN-FOOTER-01](/invoca-design-system/patterns/footers#constraints) — the footer goes on the surface whose boundary matches what it saves, and most attribute viewing has no whole-page commit to hold one at all. |
