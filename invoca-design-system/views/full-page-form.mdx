...
several independent sections, this archetype resolves [CRUD: Create](/invoca-design-system/patterns/crud/create#constraints)'s
own "a Table of Contents" into a concrete in-page anchor nav — see [Contents — the form](#contents-the-form)
below.
...
|---|---|---|
| Sections that must be completed in a fixed, dependent order | [Wizard](/invoca-design-system/views/wizard) | Sections that depend on each other in order need a visible sequence, not a scrollable page — CRUD's own Create page states this rule directly. |
...
like a page-level action, and `Right` is where page-level actions live. But `Right` sits above a
form the reader hasn't finished filling in — a Save button there implies the commit is available
...
the sections and a way to move between them — resolving what CRUD's own Create page already
...
Cancel and the commit action, commit last. This archetype is a single unit of work, not a step
in a sequence, so the leading-and-trailing arrangement with a `Back` action doesn't apply here —
that's [Wizard](/invoca-design-system/views/wizard)'s footer, not this one.

| Slot | Use | Constraint |
|---|---|---|
| Trailing — commit and cancel | `Button` | Cancel first, commit last. At most one primary. Commit label states what's being created — "Create Campaign," not "Save" or "Submit," the same rule CRUD's own Create page states. |
...
<Note>
  **The anchor nav is a convenience, not a route.** A region dropped or collapsed at a narrow
  viewport is never the only path to a function — scrolling reaches every section with or
  without the nav, so collapsing it costs navigation speed, not access.
</Note>
...
|---|---|---|
| **TITAN-FPFORM-01** | The page-level commit and cancel live in `Footer`, never in `Header` — `Right`. | `Right` holds page-level actions, but a form's commit isn't available until the reader has finished, and `Footer` is where the frame already places commit and cancel — see [What fills each region — Header — Right](#header-right). |
| **TITAN-FPFORM-02** | Several independent sections render on one scrollable page with an anchor nav, never behind tabs or a step sequence. | Independent sections don't need linear traversal, and hiding them behind tabs reintroduces the sectioned-page signal this archetype isn't — the same rule CRUD's own Create page states. |
| **TITAN-FPFORM-03** | The footer uses the trailing-only arrangement. The leading-and-trailing arrangement with a `Back` action is reserved for a step in a sequence. | This archetype is one unit of work, not a step, so there's no previous step to return to. |
| **TITAN-FPFORM-04** | The commit button's label states what's being created or saved, never a generic verb. | "Create Campaign," not "Submit" — the same rule CRUD's own Create page states. |
...
| **TITAN-FPFORM-06** | Closing or navigating away from a non-empty, unsubmitted form warns before discarding it, per [Destructive confirmation](/invoca-design-system/patterns/destructive-confirmation)'s existing unsaved-edits guidance. | This archetype doesn't re-decide that behavior, it inherits it — the same way CRUD's own Create page does for its own surface. |
