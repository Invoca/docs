...
**One primary action, and it creates a member of the collection.** Which surface that action
...
[Known issues](#known-issues) for the gap between that design and what code ships today.
...
<Note>
  **Which columns collapse first is a per-page decision, not this archetype's.** The archetype
  requires that *something* collapses before the table forces horizontal scrolling — a dropped
  region is never the only route to a function, so a collapsed column's data must stay reachable
  from the row even once it's off the grid.
</Note>
...
- **Sort state is announced on change**, stated as a plain sentence — "Sorted by calls,
  descending" — not inferred from an arrow glyph on the column header alone, since a
  visual-only signal is never sufficient.
...
- **A row action's accessible name includes the row's identifying value** — "Delete Q3 Paid
  ...
  actions can tell them apart without cross-referencing the visual row. The same
  column-of-identical-controls problem Checkbox's own page already states for a row of
  selection controls.
...
| **TITAN-LSTVIEW-04** | Bulk actions render in the table's own selection bar, never in the header. | The header has no selection-count context; the table does — see [Known issues](#known-issues) for the gap between that design and what code ships today. |
