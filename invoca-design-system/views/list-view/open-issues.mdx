...
- **Two grid exports ship; the system picks `DataGridPremium`.** Both `DataGridPro` and
  `DataGridPremium` are in the package. `DataGridPremium` is the one to reach for on a list
  page — see [Table](/invoca-design-system/components/data-display/table#vocabulary). The
  guidance story itself still says "use the grid" without naming which; that story needs the
  same update.
