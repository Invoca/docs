...
| Slot | Use | Constraint |
|---|---|---|
| Actions | `Actions` | At most one **primary** button. Three actions maximum; collapse the rest into a [Menu](/invoca-design-system/components/actions/menu). |
...
| Sections | **Tabs** | A tab bar | `Tabs`, composed outside `Header` — see [Known issues](/invoca-design-system/views/overview/open-issues#known-issues) |
| Filters | **Filters** | A filter row | `Controls`, a separate region — see [Known issues](/invoca-design-system/views/overview/open-issues#known-issues) |
...
reloaded, and reached with the back button. A tab switches which
...
{/* Renders outside Header today (see Known issues). Inset it to the header's gutter. */}
<Tabs
  value={tab}
...
the states a tab carries, the id pairing the shipped `TabPanel` gets wrong, what "tab bar" and
"tab" name on each side, and the vertical orientation, which is a navigation rail rather than a
tab bar with no region of its own yet.
...
| — when the header has no tab bar | — | Design intent puts the filters **in the header**; code has no second row, so `Controls` is where they land — see [Known issues](/invoca-design-system/views/overview/open-issues#known-issues). |
...
**Whether the page gets a footer at all, and whether the footer belongs to the page or to a
container inside it, is decided by [Footers](/invoca-design-system/patterns/footers).** This
section governs the region itself once that decision lands on the page.
...
| Trailing — commit and cancel | `Button` | Trailing-primary order. At most one primary. |
...
<Note>
  **The leading slot is a convention, not a prop.** It is produced by setting
  ...
  tertiary, leftmost" — see [Known issues](/invoca-design-system/views/overview/open-issues#known-issues).
</Note>
...
<Warning>
  **Wrapping the trailing group costs you the footer's gap.** The footer spaces its *direct*
  ...
  example uses a margin on a button to do it — which is exactly what region spacing coming only
  from the frame says not to do.
...
| **Commitment is deliberate, escape is easy** | The primary action sits last, dismissal is lightest and first. A page that is easy to leave and deliberate to submit. |
...
this for you and the shipped panel guesses wrong — see [Known issues](/invoca-design-system/views/overview/open-issues#known-issues).
...
see [Known issues](/invoca-design-system/views/overview/open-issues#known-issues).
