...
| Slot | Use | Constraint |
|---|---|---|
| Leading | — | Empty. A backward action belongs to a sequence, not to standing configuration — [TITAN-VIEW-10](/invoca-design-system/views/overview#constraints). |
| Trailing — Cancel | `Button` **tertiary** | Discards every uncommitted edit on the view. |
| Trailing — commit | `Button` **primary** | Labeled for the action, never the section — "Save changes". At most one primary on the view, per [TITAN-BTN-01](/invoca-design-system/components/actions/button#constraints). |
...
- **The footer's commit and cancel carry accessible names that name the action, not the
  ...
  for the whole view and nothing to disambiguate it from. See
  [TITAN-FOOTER-06](/invoca-design-system/patterns/footers#constraints).
- **A failed commit moves focus to the first invalid field**, in its own section, and the
  ...
  off-screen from the button that was pressed — see
  [TITAN-FOOTER-11](/invoca-design-system/patterns/footers#constraints).
...
| **TITAN-SETVIEW-07** | A failed commit keeps every edited field in its unsaved state, with the error rendered against the section it came from. It never silently reverts to the last-saved value. | Same reasoning as [TITAN-INLINE-05](/invoca-design-system/patterns/inline-editing#constraints) — reverting on failure discards a correction the reader believed they'd made, with no way to recover it. |
