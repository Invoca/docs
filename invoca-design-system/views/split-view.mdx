...
**This archetype adds no regions, but it changes how one is used.** A page has exactly one
...
**Selection changes the URL.** The frame requires this for tabs because a section that exists
only in local state cannot be linked, survive a reload, or be reached by the back button. The
same reasoning applies to a selected
...
|---|---|---|
| **TITAN-SPLITVIEW-01** | The list pane and detail pane share one `Contents` region. They are never modeled as two separate `Contents`. | One `Contents` per page, carrying the `main` landmark once. |
...
| **TITAN-SPLITVIEW-04** | Selecting a row changes the URL to a state that resolves back to that same selection on reload or when shared. | Matches the same reasoning tabs already follow — a selection that exists only in local state cannot be linked, reloaded, or reached by the back button. |
| **TITAN-SPLITVIEW-05** | Below the two-pane breakpoint, the split collapses to the list pane alone. The detail pane never squeezes into an unreadable sliver instead of collapsing. | A region dropped at a narrow viewport is never the only route to a function, so the detail content must still be fully reachable, just through a different surface. |
